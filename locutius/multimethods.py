...
def multimethod(previous_multi=None, dispatch_key=None):
    ...
    The default value for 'previous_multi' is None. This is hard (maybe
    impossible) to trigger. If a multimethod decoration is applied without
    arguments,

    , an invalid value. An exception will be raised if multimethod
    doesn't have a 'previous_multi'.
...
def method(some_type, *args):
    """Replace another function f at decoration time with a multimethod bound to
    'some_type.' This is a one-step alternative to the two-step procedure of
    declaring a multi and then defining a multimethod with that multi and a
    type.
    ...
    a hidden dispatch table is consulted with the type of the argument,
    namely 'Circle', and the resulting multimethod is called.
    ...
    def _method_inner(fn):
        ...
        if previous_multi is None or not hasattr(previous_multi, '__multi__'):
            ...
            # make the multi if it doesn't exist:

            previous_multi = multi(get_dispatch_value)

            # call with some_type == None just to set aggressive default
