...
def multi(get_dispatch_value):
    ...
    If the argument 'get_dispatch_value' is not explicitly supplied, then it is
...
def multimethod(dispatch_fn=None, dispatch_key=None):
    """Decorates a target function f so that its dispatch table is consulted
    with the given 'dispatch_key' later when f is called. The 'dispatch_fn' must
    be the target of a previous 'multi'. The present function, 'multimethod',
    adds an entry to the dispatch table that 'multi' creates.

    Use 'dispatch_key=None' to set a default multimethod for f for use cases
    where a particular dispatch_key cannot be found in f's dispatch table.
...
def method(dispatch_value, *args):
    """Returns a new closure, __method_inner, that takes another function -- the
    decorated function -- as argument. The closure '__method_inner' will be
    called at decoration time with the decorated (target) function, f, as
     argument.

    The purpose of 'method' is to bind _the name of_ f to a type. We want to end
    up with multiple particular functions, all with the same name, but bound to
    different types. This is easiest to explain by example.
    ...
    Decoration time for method proceeds in two steps. First, 'method' is called
    with argument 'Circle', which is a type that becomes the 'dispatch value'
    for this particular instance of 'area'. 'method' returns an inner
    closure, '_method_inner', over 'dispatch_value'.

    In the second, and final, step of decoration time, the inner closure is
    immediately called with the decorated function, 'area', as argument. The
    decoration mechanism replaces the implementation of 'area' with the
    particular instance of closure '_method_inner'. User's calls of 'area'
    with arguments of type 'Circle' will now go to that instance closure of
    '_method_inner'.

    Later, when 'area' is called with a Circle, the instance closure
    __method_inner first looks up the name 'area' of function 'area' in a global
    __locutius_multimethods__ dictionary, and doesn't find it (apparently,
    the Python standard 'globals' dictionary does not contain an entry for name
    'area', at least in the tested version of Python 3.6.3, using, instead, the
    name 'fn', which is the name of the function parameter to __method_inner).
    __method_inner will now create and return a dispatch function for the pair
    'area' and 'Circle', in three steps:

    1. call 'multi' with 'get_dispatch_value' as argument. 'multi' returns an
    inner closure named '_multi_inner'.

      1.a. Because 'method' is only for dispatch-on-type,
      this 'get_dispatch_value' is a fixed function that returns the type of
      its first argument and discards the rest of its arguments. Multi closes
      over 'get_dispatch_value' inside '_multi_inner', saving it for later use.
